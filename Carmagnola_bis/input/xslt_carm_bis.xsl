...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="mode" select="'reading_ma'"/>
    ...
    <xsl:template match="tei:add[@place, 'above']/tei:del" mode="interlinear">
        <xsl:variable name="font-size">
            <xsl:choose>
                <xsl:when test="number(substring-after(parent::tei:zone/@style, 'font-size:20'))"
                    >14</xsl:when>
                <xsl:otherwise>10</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <tspan
            style="text-decoration:line-through;font-size:{if (contains(ancestor::tei:zone/@style, 'font-size:20')) then $font-size else '14'}px;font-family:Verdana"
            dy="{if (preceding-sibling::tei:del[tei:add[@place='above'] | preceding-sibling::tei:add[@place='above']] ) then -($font-size+12)  else -($font-size+12)}"
